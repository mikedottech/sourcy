...
### ANTLR 3 tree parsers must be built at `-O1`

The C code generated by **ANTLR 3** (last released in 2014) contains constructs — large switches over imaginary `<UP>` / `<DOWN>` tokens, implicit returns in long functions, casts between tree adaptor subtypes — that modern Clang and GCC mis-optimize at `-O2` and above. The symptom is a bogus runtime error during AST walking:

```
... error 3 : cannot match to any predicted input
... error 9 : org.antlr.runtime.UnwantedTokenException, near (...) : Extraneous input - expected <UP>
ERROR: Incomplete state controller. Missing attributes for the controller <NAME>.
```

The bug **only appears in Release builds** and **only on inputs that exercise certain controller patterns** (the `cloudi.cns` / `cloudi.cmd` pair under `tests/cns/` reproduces it). AddressSanitizer and UndefinedBehaviorSanitizer both come back clean, so it's neither a memory error nor a UB class either tool covers; it's a real optimization-vs-code-generator incompatibility.

The fix lives in `CMakeLists.txt`: the three ANTLR3-generated tree parsers (`cnscheck.c`, `luagen.c`, `cppgen.c`) are pinned to `-O1` via `set_source_files_properties(... COMPILE_OPTIONS "-O1")`. The combined lexer+parser files (`cnsLexer.c`, `cnsParser.c`, `defLexer.c`, `defParser.c`) are left at the project's default optimization level (they're not tree parsers and don't exhibit the issue. Performance impact is negligible because parsing/walking is not the hot path).

The workaround is currently scoped to non-MSVC compilers (GCC / Clang / AppleClang). MSVC has not been observed to exhibit the bug; if it ever is, the same `set_source_files_properties` call needs a different flag (`/Od` or `/O1` (note that MSVC's `/O1` has different semantics from GCC's)).

If you ever migrate the project to **ANTLR 4**, this workaround can be removed: ANTLR 4's runtime is C++, has continued maintenance, and the code-generator does not produce the patterns that trigger the bug.
